...
SF               = false;
SF_efficiency    = 0; % par.eta
sp_r             = 1000; % SRV [cm/s]
nemitter_EF0     = -4.026; % [eV]
rightelectrode   = -4.076; % [eV]
...
% -------------------------------------------------------------------------
% Options (as implemented below): 'charges', 'junction_depth', 'SF',
% 'eta_sf', 'sp_r', 'nemitter_EF0', 'rightelectrode
sweepParam  = 'rightelectrode'; % <--- change this
sweepValues = [-4.076,-5.076]; % <--- and this (example: sweep charges 0 and 1e19)
...
for iSweep = 1:numel(sweepValues)
    ...
    switch sweepParam
        case 'charges'
            ...
            par = add_charged_layer(par, charges);
        ...
        case 'nemitter_EF0'
            ...
            par.EF0(3:5) = repmat(nemitter_EF0, 1, 3); % change to (3) if no charged layer
...
function par_out = add_charged_layer(par_in, charges)
    thickness_nm        = 1.5;      % charged layer thickness [nm])
