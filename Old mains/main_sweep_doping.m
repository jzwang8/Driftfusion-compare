...
% -------------------------------------------------------------------------
file_name = 'Input_files/pn_junction_doping.csv';
run_name = 'ch0_SF0_eta0_ref0p15_sp1000';

% --- Base SF/charged layer settings (defaults) ---
charges_base          = 0;        % [cm^-3]
junction_depth_base   = 300; % [nm]
Singlet_Fission_base  = false;
SF_efficiency_base    = 0;        % par.eta
sp_r_base             = 1000; % SRV [cm/s]
nemitter_EF0_base      = 0;
rightelectrode_base    = -4.076;
...
% kappa_vals = Reflection_tmm; % toggle this if using TMM reflection

% other parameters to change but not sweep
thickness        = 1.5;      % charged layer thickness [nm])
...
% 'eta_sf', 'reflection', 'sp_r', 'nemitter_EF0', 'rightelectrode
sweepParam  = 'rightelectrode';              % <--- change this
sweepValues = [-4.076];              % <--- and this (example: sweep charges 0 and 1e19)
...
% also push into base so the existing code below still works
...
for iSweep = 1:numel(sweepValues)
    ...
    sp_r            = sp_r_base;
    nemitter_EF0    = nemitter_EF0_base;
    rightelectrode   = rightelectrode_base;
    ...
    % if sweeping doping, need to also modify mobilities
    % par.EF0(3:5) = repmat(nemitter_EF0, 1, 3);
    % N_d = getDopingConcFromEf(nemitter_EF0);
    % [mu_n, mu_p] = getMobilitiesFromDopingConc(N_d);
    % par.mu_n(3:5) = repmat(mu_n, 1, 3);
    % par.mu_p(3:5) = repmat(mu_p, 1, 3);
    ...
    % Add charges into Extra_charge in par
...
% -------------------------------------------------------------------------
runs = evalin('base','runs');   % get final runs from base
