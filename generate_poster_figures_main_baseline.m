function generate_poster_figures_main_baseline(summaryFile)
...
% Baseline (no SF layer)
mask_base_A = (T_success.SF_flag == 0) & ...
              (T_success.SRV_cm_s == optimal_SRV) & ...
              (abs(T_success.EF_emitter_eV - optimal_EF) < tol) & ...
              (T_success.junction_depth_nm == optimal_jd) & ...
              (abs(T_success.Phi_R_offset_eV - 0) < tol);
if any(mask_base_A)
    run_idx = T_success.run_number(find(mask_base_A, 1));
    r = runs(run_idx);
    h_base_A = plot(r.EQE.wavelengths, r.EQE.EQE, 'k--', ...
                    'LineWidth', 1.5);
end
...
if exist('h_base_A', 'var')
    legend([h_eta; h_base_A], [leg_labels_eta, {'Baseline'}], ...
...
% Baseline (no SF layer) at each SRV — dashed, same color
for s = 1:length(SRV_list)
    mask_base = (T_success.SF_flag == 0) & ...
                (T_success.SRV_cm_s == SRV_list(s)) & ...
                (abs(T_success.EF_emitter_eV - optimal_EF) < tol) & ...
                ...
                (abs(T_success.Phi_R_offset_eV - 0) < tol);
    if any(mask_base)
        run_idx = T_success.run_number(find(mask_base, 1));
        r = runs(run_idx);
        plot(r.EQE.wavelengths, r.EQE.EQE, '--', 'LineWidth', 1.5, ...
             'Color', SRV_colors(s, :), 'HandleVisibility', 'off');
    end
end

% Dummy handles for legend: solid = SF, dashed = baseline
...
xlim([400 800]);
ylim([50 120]);
legend([h_srv; h_leg_solid; h_leg_dashed], ...
       [arrayfun(@(x) sprintf('s_p = %.0d', x), SRV_list, 'UniformOutput', false), ...
        {'SF (\eta=2)', 'Baseline'}], ...
       'Location', 'southeast', 'FontSize', 13);
set(gca, 'FontSize', 13, 'Box', 'on');
grid on;
...
% Baseline (no SF layer) at each junction depth — dashed, same color
for jj = 1:numel(jd_vals_panel)
    mask_base = (T_success.SF_flag == 0) & ...
                (T_success.SRV_cm_s == 10000) & ...
                (abs(T_success.EF_emitter_eV - optimal_EF) < tol) & ...
                (T_success.junction_depth_nm == jd_vals_panel(jj)) & ...
                (abs(T_success.Phi_R_offset_eV - 0) < tol);
    if any(mask_base)
        run_idx = T_success.run_number(find(mask_base, 1));
        r = runs(run_idx);
        plot(r.EQE.wavelengths, r.EQE.EQE, '--', 'LineWidth', 1.5, ...
             'Color', jd_colors(jj, :), 'HandleVisibility', 'off');
    end
end
...
legend([h_jd; h_leg_solid_C; h_leg_dashed_C], ...
       [leg_labels_jd, {'SF (\eta=2)', 'Baseline'}], ...
       'Location', 'southeast', 'FontSize', 13);
...
% Baseline (no SF layer) at each doping level — dashed, same color
for nn = 1:numel(ND_vals_panel)
    mask_base = (T_success.SF_flag == 0) & ...
                (T_success.SRV_cm_s == optimal_SRV) & ...
                (abs(T_success.EF_emitter_eV - EF_vals_panel(nn)) < tol) & ...
    ...
    if any(mask_base)
        ...
        r = runs(run_idx);
        plot(r.EQE.wavelengths, r.EQE.EQE, '--', 'LineWidth', 1.5, ...
             'Color', ND_colors(nn, :), 'HandleVisibility', 'off');
    end
end
...
legend([h_nd; h_leg_solid_D; h_leg_dashed_D], ...
       [leg_labels_nd, {'SF (\eta=2)', 'Baseline'}], ...
       'Location', 'southeast', 'FontSize', 13);
...
% Baseline (no SF layer) at same SRV
mask_base_phi = (T_success.SF_flag == 0) & ...
                (T_success.SRV_cm_s == SRV_plot) & ...
                (abs(T_success.EF_emitter_eV - optimal_EF) < tol) & ...
                (T_success.junction_depth_nm == optimal_jd) & ...
                (abs(T_success.Phi_R_offset_eV - 0) < tol);
if any(mask_base_phi)
    run_idx = T_success.run_number(find(mask_base_phi, 1));
    r = runs(run_idx);
...
if exist('h_base_phi', 'var')
    legend([h_phi; h_base_phi], [PhiOff_labels, {'Baseline'}], ...
