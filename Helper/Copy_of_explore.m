...
%
    methods (Static)
            ...
            parfor i = 1:n1
                ...
                for j = 1:n2
                    try
                        ...
                        n_av_vec(j)     = mean(sol_Voc.u(end, par.pcum(2):par.pcum(5),1));
                        p_av_vec(j)     = mean(sol_Voc.u(end, par.pcum(2):par.pcum(5),2));
        ...
        % second dimension so all plot functions work.
            ...
            parfor i = 1:nvals
                ...
                try
                    ...
                    n_av         = mean(sol_Voc.u(end, par.pcum(2):par.pcum(5),1));
                    p_av         = mean(sol_Voc.u(end, par.pcum(2):par.pcum(5),2));
        ...
        function plotPL(exsol)
            % For 2-param: surf(parval2, parval1, PLint(:,:,end))
            % For 1-param (n2=1): this is a thin strip in parval2.
            figure(100)
        ...
        %
        % If you want, we can next update explore1par to also store those
        % profiles in full 3D so these work for 1-parameter runs too.
        ...
        function plotfinalELx(exsol)
            ...
            for i=1:length(exsol.parval1)
                for j = 1:length(exsol.parval2)
                    ...
                    % You would then call your EL plotter here
                end
            end
        end
        ...
        function plotCE(exsol_Voc, exsol_eq, xlogon, ylogon, zlogon, normalise)
            % As in original; still fundamentally 2-param (thickness & light).
            % For a 1-param run with n2=1 it degenerates accordingly.
